...
def main(logger, args):
    ...
    if not args.do_zeroshot:
        ...
        checkpoint = None
    metaicl_model = MetaICLModel(logger, args.out_dir, device=args.device)
...
def run(logger, task, metaicl_data, metaicl_model, train_data, dev_data, seed,
    ...
    if os.path.exists(cache_path):
        ...
        if args.interpret:
            losses = metaicl_model.do_interpret(metaicl_data, args.test_batch_size, verbose=True, zero_baseline=args.zero_baseline)
        else:
            losses = metaicl_model.do_inference(metaicl_data, args.test_batch_size, verbose=True, zero_baseline=args.zero_baseline)
...
if __name__=='__main__':
    ...
    parser.add_argument("--interpret", default=False, action="store_true", help="Storing data used for T3-Vis case study.")
    parser.add_argument("--device", default=None, type=str)
    parser.add_argument("--zero_baseline", default=False, action="store_true")
    ...
    try:
        ...
        Exception(f"Error with Device Name\"{args.device}\"")
